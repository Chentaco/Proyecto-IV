...
class JugadorTestCase(TestCase):
	...
	def test_jugador_pertenece_team(self):
		...
		Iniesta = Jugador.objects.get(nick="Andres", name="Iniesta")
		Biraghi = Jugador.objects.get(nick="Cristiano", name="Biraghi")
...
class RutasTeamsJSON(APITestCase):
	def test_listar_teams(self):
		...
		response = self.client.get('/match/teams/json/teams/')
	...
	def test_detalle_team(self):
		...
		for i in teams:
			response = self.client.get('/match/teams/json/teams/'+str(i)+'/')
			self.assertEqual(response.status_code, status.HTTP_200_OK)
			self.assertEqual(response['Content-Type'], 'application/json')
